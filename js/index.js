  ...
  api
    ...
    .then(responseJson => {
      ...
      for (let i = 0; i < responseJson.results.length; i++) {
        ...
        if (
          companyName.indexOf($('.search').val() >= 0) &&
...
function renderData(json) {
  ...
  $('.news').before(
    `<section role='region' class='news-header'><h3>News About ${companyName}</h3></section>`
  );
...
function renderCompanyDrugList(companyName) {
  api
    .getFdaData('openfda.manufacturer_name', companyName, displayApiError, 10)
    .then(response => response.json())
    .then(responseJson => {
      $('.drugs').before(
        `<section role='section' class='drugs-header'><h3>Medicines made by ${companyName}</h3></section>`
      );
      for (let i = 0; i < responseJson.results.length; i++) {
        $('.drugs').append(
